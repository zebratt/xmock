...
export class AppModelClass {
    ...
    async saveFileContent() {
        const res = await ApiService.post<IResponse<any>>(`${HOST}/api/files/${this.currentFileId.get()}/save`, {
    ...
    async loadFileContent(id: number) {
        const res = await ApiService.get<IResponse<string>>(`${HOST}/api/files/${id}`)
